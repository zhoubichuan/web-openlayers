...
export default {
  data() {
    return {
      selected: window.localStorage.geojson || "line-samples.geojson",
    }
  },
  async mounted() {
    let {
    ...
    let res = await axios.get(
      this.$withBase(
        `/data/geojson/${window.localStorage.geojson || "line-samples.geojson"}`
      )
    )
  ...
  methods: {
    getDataSource(e) {
      this.selected = window.localStorage.geojson = e.target.value
    },
  },
}
</script>
