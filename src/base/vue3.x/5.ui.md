...
export default {
  data() {
    return {
      selected: _localStorage.geojson || "line-samples.geojson",
    }
  },
  async mounted() {
    let {
    ...
    let res = await axios.get(
      this.$withBase(
        `/data/geojson/${_localStorage.geojson || "line-samples.geojson"}`
      )
    )
  ...
  methods: {
    getDataSource(e) {
      this.selected = _localStorage.geojson = e.target.value
    },
  },
}
</script>
